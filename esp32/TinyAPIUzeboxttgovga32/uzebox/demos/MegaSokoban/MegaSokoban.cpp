...
#include "data/sokoban_tiles.cpp"
...
static void sokoban_load_level() {
	...
	token = uze_pgm_read_byte(reading_pos);
	reading_pos++;
	//JJ level_width = ((token & 0b11110000) >> 4) + 1;
	level_width = ((token & 0xF0) >> 4) + 1;	
	//JJ level_height = (token & 0b00001111) + 1;
	level_height = (token & 0x0F) + 1;
	...
	do {
		token = uze_pgm_read_byte(reading_pos);
		//JJ token1 = ((token & 0b11110000) >> 4);
		token1 = ((token & 0xF0) >> 4);		
		//JJ token2 = (token & 0b00001111);
		token2 = (token &0x0F);

		if (token1 < 0xe) 
        {
			//JJ text_putc(FONT_SOKOBAN_FLOOR + (token1 & 0b00000111));
			text_putc(FONT_SOKOBAN_FLOOR + (token1 &0x07));
			if (token1 > 0x7) 
            {
				//JJ text_putc(FONT_SOKOBAN_FLOOR + (token1 & 0b00000111));
				text_putc(FONT_SOKOBAN_FLOOR + (token1 & 0x07));
		...
		if (token2 < 0xe) {
			//JJ text_putc(FONT_SOKOBAN_FLOOR + (token2 & 0b00000111));
			text_putc(FONT_SOKOBAN_FLOOR + (token2 & 0x07));
			if (token2 > 0x7) 
            {
				//JJ text_putc(FONT_SOKOBAN_FLOOR + (token2 & 0b00000111));
				text_putc(FONT_SOKOBAN_FLOOR + (token2 & 0x07));
	...
	//JJ} while ((token1 != 0b00001111) && (token2 != 0b00001111));
...
static void sokoban_undo_last_push() {
	...
	if (sokoban_undo_ringbuffer_count > 0) {
		...
		//JJ a = vram + sokoban_undo_ringbuffer[sokoban_undo_ringbuffer_pos].pos;
		a = (unsigned char *)vram + sokoban_undo_ringbuffer[sokoban_undo_ringbuffer_pos].pos;
...
static void DrawMap2WithTileOffset(unsigned char x,unsigned char y,const char *map, const char offset){
	unsigned char i;
	unsigned char mapWidth=uze_pgm_read_byte(&(map[0]));
	unsigned char mapHeight=uze_pgm_read_byte(&(map[1]));

	for(unsigned char dy=0;dy<mapHeight;dy++){
		for(unsigned char dx=0;dx<mapWidth;dx++){

			//i=uze_pgm_read_byte(&(map[(dy*mapWidth)+dx+2]));
			unsigned int auxCalc= (dy*mapWidth)+dx+2;
			i=uze_pgm_read_byte(&(map[auxCalc]));
            
			//vram[((y+dy)*VRAM_TILES_H)+x+dx]=(i + RAM_TILES_COUNT + offset) ;
			auxCalc= ((y+dy)*VRAM_TILES_H)+x+dx;
			vram[auxCalc]= (i + RAM_TILES_COUNT + offset) ;
		}
	}
...
void do_megasokoban_Setup()
{
 ClearScreen(); //Borro SDL surface
     
 RAM_TILES_COUNT=0;
 FIRST_RENDER_LINE= 20; //Realidad es 35
 FRAME_LINES= 224; //Realidad FRAME_LINES SCREEN_TILES_V*TILE_HEIGHT
 SCREEN_TILES_V= 12;
 //SCREEN_TILES_H = 12;
 SCREEN_TILES_H = 16;
 VRAM_TILES_H = 16;
 ...
 gb_uze_video_cur_mode=10; //240x224 resolution, 6x8 pixels tiles, 40x28 vram
