...
void do_BitmapDemo_Setup()
{
 ...
 gb_uze_video_cur_mode=8; //Modo de video 8 forzamos 
 ...
 // Serial.printf("do_BitmapDemo_Setup\n");
 //#endif  
 WaitVsync(50);//Esperamos por sincronizacion   
}

//JJ int main()
int do_BitmapDemo_main()
{
	//Clear the screen (fills the vram with tile zero)
	//ClearVram();
	...
	while(1)
    {
		...
		for(int j=0;j<(SCREEN_WIDTH/4)*SCREEN_HEIGHT;j++){
			vram[j]=uze_pgm_read_byte(&(uzeboxlogo[j]));
		}
		...
		palette[1]=uze_pgm_read_byte(&(sprite_palette[1]));
		palette[2]=uze_pgm_read_byte(&(sprite_palette[2]));
		palette[3]=uze_pgm_read_byte(&(sprite_palette[3]));
			...
			if((rand()&1)==0){
				for(u8 y=0;y<sprite_height;y++){
					for(u8 x=0;x<sprite_width;x+=4){
						u8 pix=uze_pgm_read_byte(&(sprite[(y*(sprite_width/4))+(x/4)]));
